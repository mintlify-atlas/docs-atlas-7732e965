...
<CodeGroup>
```bash GET /emails
curl -X GET "https://api.example.com/emails?q=subject=like='Research%'" \
...
<CodeGroup>
```bash GET /emails/{id}
curl -X GET "https://api.example.com/emails/123" \
  -H "Authorization: Bearer {token}"
```
</CodeGroup>
...
<CodeGroup>
```bash PUT /emails/{id}
curl -X PUT "https://api.example.com/emails/123" \
  -H "Authorization: Bearer {token}" \
  -H "Content-Type: application/json" \
  -d '{
...
curl -X DELETE "https://api.example.com/emails/123" \
  -H "Authorization: Bearer {token}"
```
</CodeGroup>
...
curl -X GET "https://api.example.com/emails/123/send" \
  -H "Authorization: Bearer {token}"
```
</CodeGroup>
...
// Send email
const sendResponse = await fetch(`https://api.example.com/emails/${created.id}/send`, {
...
send_response = requests.get(
    f'https://api.example.com/emails/{email_id}/send',
    headers={'Authorization': f'Bearer {token}'}
)
